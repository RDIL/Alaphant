...
abstract class CheckMatcherTask : DefaultTask() {
    ...
    private fun scoreMembers(score: Score, old: MatchSide, fixture: Scramble, new: MatchSide, result: MatchResult) {
        for (node in old.nodes) {
     ...
     */
    private fun scoreTransfer(score: Score, old: MatchSide, new: MatchSide, fixture: Scramble, result: MatchResult) {
    ...
    fun writeTo(file: File) {
        ...
        ZipOutputStream(file.outputStream().buffered()).use { zip ->
            for (node in nodes) {
                ...
                zip.putNextEntry(ZipEntry(classes.getValue(node.name) + ".class"))
